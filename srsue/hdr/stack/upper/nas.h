...
class nas : public nas_interface_rrc, public srsran::timer_callback, public nas_base, public test
{
public:
  ...
  // Flag for RRC
  bool rrc_release_flag = false;

  bool set_test_config();
...
private:
  ...
  // Values according to TS 24.301 Sec 10.2Error: binding transmitter socket (tcp://*:20000): Address already in use

  const uint32_t t3402_duration_ms          = 12 * 60 * 1000; // 12m
  const uint32_t t3410_duration_ms          = 10 * 1000;      
  // const uint32_t t3411_duration_ms          = 45 * 1000;
  // const uint32_t t3421_duration_ms          = 45 * 1000;
  // const uint32_t reattach_timer_duration_ms = 45 * 1000;
  ...
  const uint8_t ue_svn_oct1 = 0x0;
  const uint8_t ue_svn_oct2 = 0x1;

  // Security
  bool eia_caps[8] = {};
  bool eea_caps[8] = {};
  bool origin_eia_caps[8] = {};
  bool origin_eea_caps[8] = {};

  // custom value
  bool emergency_attach = false;
  bool sec_hdr_plain = false;
  int auth_fail_cause = 0;
  bool attach_mac_zero = false;
  bool attach_plain = false;
  bool verify_sr = true;

  //Testing flag
  bool keep_attach = true;  // keep attach or terminate when testing procedure is done (-e option of controller.py)
  bool testing = false;
  bool verify = false;
  bool expire_flag = false;  // check timer expire
  bool attach_success = false;
  bool run_flag = false;
  bool first_attach = true;  // first attach of testing procedure (keep TRUE)
  bool auth_ksi_fresh = true;  // On testing, KSI updates to new value
  bool using_victim_key = false; // using victim key for testing procedure (use TRUE to make a normal GUTI attach)
  bool test_guti_save = false;  // Save test attach result GUTI. (If false, save initial GUTI, for verification attach)
  bool first_detach = false;  // Run or Ignore detach request on initial attach (Don't touch)
  bool root_first_detach = false;  // For saving first_detach flag value. Set only this flag for first_detach flag value (If this flag is set to false, the detach request is not sent during initial attach.)
  bool ctxt_save = true;  // rewrite ctxt file when initial attach is finished
  bool ctxt_read = true;  // reread ctxt file when verify attach is started
  bool check_no_response = false; // Set True when core send the response to UE
  // tmsg_args_t tmsg;

  //Testing Mode
  std::string testing_mode = "single_usrp";
  std::string testing_role = "attacker";
  std::string testing_state = "IDLE";
  ...
  void start_tau_request(srsran::establishment_cause_t cause_);
  ...
  //logger
  void logging_dl_message(std::string file, std::string message, std::string notes="");
  void logging_ul_message(std::string file, std::string message, std::string notes="");
  void logging_message(std::string file, std::string message, std::string notes="");
  void send_end_indicator(std::string file);
  std::ofstream nas_log_stream;
  //control test message
  bool control_test_message();

  // Parsers
  void parse_attach_accept(uint32_t lcid, srsran::unique_byte_buffer_t pdu, const uint8_t sec_hdr_type);
  ...
  //make new function
  void send_security_mode_complete(bool imeisv_present);
