...
static std::atomic<bool> initial_running        = {true};
static std::atomic<bool> testing_running        = {true};
...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    //custom args
    ("nas.detach", bpo::value<bool>(&args->do_detach)->default_value(true),  "Whether to perform Detach request")
    ("nas.emergency_attach", bpo::value<bool>(&args->stack.nas.emergency_attach)->default_value(false),  "Whether to perform Emergency Attach request")
    ("nas.sec_hdr_plain", bpo::value<bool>(&args->stack.nas.sec_hdr_plain)->default_value(false),  "Whether to perform Plain security header")
    ("nas.auth_fail_cause", bpo::value<int>(&args->stack.nas.auth_fail_cause)->default_value(0), "Hex value of Auth fail cause")
    ("nas.attach_mac_zero", bpo::value<bool>(&args->stack.nas.attach_mac_zero)->default_value(false),  "Put Zero on MAC of Attach request")
    ("nas.attach_plain", bpo::value<bool>(&args->stack.nas.attach_plain)->default_value(false),  "Send plain Attach request")
    ...
    ("nas.test_file",               bpo::value<string>(&args->stack.nas.tmsg.tfile_args.tfile_name)->default_value("fail"),  "Test config file name")
    ("nas.special_tag",               bpo::value<string>(&args->stack.nas.tmsg.special_tag)->default_value("none"),  "Specific tag value for one-time run")
    ("nas.keep_attach", bpo::value<bool>(&args->stack.nas.keep_attach)->default_value(true),  "Keep attach procedure without test config files")
    ("nas.default_pcap_loc", bpo::value<bool>(&args->stack.nas.default_pcap_loc)->default_value(true),  "Use default location for pcap files")
    ("nas.verify_sr", bpo::value<bool>(&args->stack.nas.verify_sr)->default_value(false),  "Using service request message for verification attach")

    //custom args
    ("nas.logfile", bpo::value<string>(&args->stack.nas.tmsg.logfile)->default_value("/tmp/nas.log"), "NAS layer log file")
    ("nas.testing_state", bpo::value<string>(&args->stack.nas.testing_state)->default_value("IDLE"), "Testing State(IDLE, CONN, DEREGI)")
    ("nas.testing_mode", bpo::value<string>(&args->stack.nas.testing_mode)->default_value("single_usrp"), "Testing Mode(single/multi USRP)")
    ("nas.testing_role", bpo::value<string>(&args->stack.nas.testing_role)->default_value("victim"), "Testing role [Attacker/Victim]")
    ("nas.controller_port", bpo::value<int>(&args->stack.nas.controller_port)->default_value(5555), "Controller port")
    ("nas.testing_ip", bpo::value<string>(&args->stack.nas.testing_ip)->default_value("127.0.0.1"), "Testing IP")
    ...
    ("usim.imsi_a", bpo::value<string>(&args->stack.usim.imsi_a), "USIM IMSI")
    ("usim.imei_a", bpo::value<string>(&args->stack.usim.imei_a), "USIM IMEI")
    ("usim.imsi_tmp", bpo::value<string>(&args->stack.usim.imsi_tmp), "Temperary IMSI")
    ("usim.tmsi_tmp", bpo::value<uint32_t>(&args->stack.usim.tmsi_tmp), "Temperary GUTI")
...
{
  cout << "********  Finish Verification Attach  ********" << endl;
  running = false;
}
// custom handler
static void initial_signal_handler()
{
  cout << "********  Finish Initial Attach  ********" << endl;
  initial_running = false;
}

static void testing_signal_handler()
{
  cout << "********  Finish Testing Attach  ********" << endl;
  testing_running = false;
}

bool check_interface(const std::string& interface_name) {
    std::string command = "ip link show " + interface_name + " > /dev/null 2>&1";
    int ret_code = std::system(command.c_str());
    if (ret_code != 0) {
        std::cout << "ip link show command did not work! (" << ret_code << ")" << std::endl;
    } else {
        std::cout << "ip link show execution completed" << std::endl;
    }
    return (ret_code == 0);
}

void add_route(const std::string& gateway, const std::string& interface) {
    if (check_interface(interface)) {
        std::string command = "sudo ip route add default via " + gateway + " dev " + interface;
        int ret_code = std::system(command.c_str());
        if (ret_code == 0) {
            std::cout << "[srsUE] Route added via " << gateway << " on " << interface << std::endl;
        } else {
            std::cout << "[srsUE] Failed to add route via " << gateway << " on " << interface << std::endl;
        }
    } else {
        std::cout << "[srsUE] Interface " << interface << " does not exist. Route not added.\n" << std::endl;
    }
}

int ping_host(const std::string& host = "8.8.8.8", int count = 4) {
    ...
    int success_count = 0;
    size_t pos = 0;
    while ((pos = output.find("bytes from", pos)) != std::string::npos) {
        ++success_count;
        pos += 10;
    }
...
int main(int argc, char* argv[])
{
  ...
  printf("[Final Version] CITesting: Systematic Testing of Context Integrity Violations in LTE Core Networks\n");
  std::string endpoint;
  int port;
  int server_fd, new_socket; 
  server_fd = new_socket = -1; // Initialize for unused socket
  struct sockaddr_in server_addr, client_addr;
  int opt = 1;
  int client_len = sizeof(client_addr);
  char buffer[1024] = {0};
  
  if (args.stack.nas.testing_mode == "multi_usrp"){ // When the process runs in "multi_usrp" mode 
    if (argc < 3) { 
      std::cerr << "Usage: " << argv[0] << "<config> ..." << std::endl;
      return -1;
    }
    printf("Testing role: %s\n", args.stack.nas.testing_role.c_str());
    printf("controller port: %d\n", args.stack.nas.controller_port);
    if (args.stack.nas.testing_role == "victim") {
      port = args.stack.nas.controller_port;
    }
    else if (args.stack.nas.testing_role == "attacker") {
      port = args.stack.nas.controller_port;
    }
    else {
      std::cerr << "Unknown role" << std::endl;
      return -1;
    }
  ...
  srsran_custom_signal_handler(initial_signal_handler);
  add_emergency_cleanup_handler(emergency_cleanup_handler, nullptr);
  srsran_debug_handle_crash(argc, argv);

  //Rename pcap file
  if (!args.stack.nas.default_pcap_loc){
    stringstream ss;
    string file_tag;
    istringstream iss;
    string pcap_file_tag;


    ss.str(args.stack.nas.tmsg.tfile_args.tfile_name);
 
    while (getline(ss, file_tag, '/')){}
    iss.str(file_tag);
    getline(iss, pcap_file_tag, '.');

    args.stack.pkt_trace.mac_pcap.filename = "/tmp/test_results/test_mac_pcap/ue_mac_" + pcap_file_tag + ".pcap";
    args.stack.pkt_trace.nas_pcap.filename = "/tmp/test_results/test_nas_pcap/ue_nas_" + pcap_file_tag + ".pcap";
    args.log.filename = "/tmp/test_results/test_ue_log/ue_" + pcap_file_tag + ".log";
  }
  // Generate time stamp to generate unique file name
  ...
  cout << "\n" << endl;
  cout << "+----------------------------------------------------" << endl;
  cout << "|  * Test File Info. * " << endl;
  cout << "|                      " << endl;
  cout << "|    - Test     File: " << args.stack.nas.tmsg.tfile_args.tfile_name << endl;
  cout << "|    - Log      File: " << args.stack.nas.tmsg.logfile << endl;
  cout << "|    - MAC Pcap File: " << args.stack.pkt_trace.mac_pcap.filename << endl;
  cout << "|    - NAS Pcap File: " << args.stack.pkt_trace.nas_pcap.filename << endl;
  cout << "|    - UE Log   File: " << args.log.filename << endl;
  cout << "|                      " << endl;
  cout << "+----------------------------------------------------" << endl;
  ...
  std::string message = "";
  printf("-----------------Testing mode----------------\n");
  printf("%s\n", args.stack.nas.testing_mode.c_str());
  if(args.stack.nas.testing_mode == "multi_usrp"){
    printf("-----------------multi_usrp----------------\n");
  ...
  {
    // *----------------------------------------------- Dual USRP Mode -----------------------------------------------*
    printf("Number of Test Cases: %d\n", args.stack.nas.tmsg.tfile_args.tfile_total);
    if(args.stack.nas.testing_mode == "multi_usrp"){
      ...
      buffer[n] = '\0';
      message = buffer;  
      std::cout << "Testing role: " << args.stack.nas.testing_role.c_str() << std::endl;
      std::cout << "Testing state: " << args.stack.nas.testing_state.c_str() << std::endl;
      ...
      if (message == "initial") {
        ...
        printf("22 running flag: %d\n", running.load());

        // Initial Attach
        cout << "\n\n" << endl;
        cout << "\t+----------------------------------------------------+" << endl;
        cout << "\t|  * Initial Attach *                                |" << endl;
        cout << "\t|                   Attaching UE...                  |" << endl;
        cout << "\t|                                                    |" << endl;
        cout << "\t+----------------------------------------------------+" << endl;
        printf("initial flag: %d\n", initial_running.load());
        if (!ue.switch_on()) {
          cout << "[debug] \'end of test\' or \'switch on error\'" << endl;
          ue.switch_off();
          break;
        }
        ...
        if (args.stack.nas.testing_state == "CONN"){
          printf("------------------------CONN------------------\n");
          printf("Send Message to Controller in CONN state\n");
          fflush(stdout);
          add_route(args.stack.nas.testing_ip, "tun_srsue1");
          sleep(2);
          int ping_count = ping_host("8.8.8.8", 1);
          std::string ping_count_str = "ping_" + std::to_string(ping_count);
          send(new_socket, ping_count_str.c_str(), strlen(ping_count_str.c_str()), 0);
          std::string response;
          response = "";
          int n = read(new_socket, buffer, 1024); //Read response from controller
          sleep(1);
          if ((n == 0 || strlen(buffer) == 0)) {
            printf("Received Empty string..\n Close the connection\n");
            break;
          ...
          buffer[n] = '\0';
          response = buffer;
          std::cout << "[CONN state] Response request: " << response << std::endl;
          add_route(args.stack.nas.testing_ip, "tun_srsue1");
          sleep(2);
          int ping_count_after = ping_host("8.8.8.8", 1);
          printf("Value of ping_count_after: %d\n", ping_count_after);
          ping_count_str = "ping_" + std::to_string(ping_count_after);
          send(new_socket, ping_count_str.c_str(), strlen(ping_count_str.c_str()), 0);
          ue.switch_off();
          finish = clock();
        ...
        pthread_create(&input, nullptr, &input_loop, &args);
        usleep(1000000);
        srsran_testing_signal_handler(testing_signal_handler);
        ...
        pthread_create(&input, nullptr, &input_loop, &args);
        usleep(1000000);
        srsran_testing_signal_handler(signal_handler);

        // Verify Attach
        cout << "\n\n" << endl;
        cout << "\t+----------------------------------------------------+" << endl;
        cout << "\t|  * Verify Attach *                                 |" << endl;
        cout << "\t|                   Attaching UE...                  |" << endl;
        cout << "\t|                                                    |" << endl;
        cout << "\t+----------------------------------------------------+" << endl;
        running = true;
  ...
  duration = (double)(finish - start) / CLOCKS_PER_SEC;
  cout << "duration:" << duration << endl; 
  raise(SIGINT); // For normal signal exit
  ue.stop();
  return SRSRAN_SUCCESS;
}
