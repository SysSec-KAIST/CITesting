...
void rrc::radio_link_failure_process()
{
  ...
  //Abort when occuring radio failure
  
  std::ofstream ofs;
  ofs.open("/tmp/radio_failure_num.log", std::ios_base::app);
  ofs << "radio_link_failure"<< std::endl;
  ofs.close();
  if (log_stream.is_open()){
    log_stream << "         : [Error] Radio_Link_Failure"<< std::endl;
  }
  else{
    printf("[Error] log stream error\n");
  }
  printf("[Error]===============Radio Link Failure====================\n");
  // raise(SIGINT); //Temperary remove raise(SIGINT) 
