...
int ue_stack_lte::init(const stack_args_t& args_)
{
  ...
  // save attacker's K and OPC
  if (32 == args.usim.k.length()) {
    ue_str_to_hex(args.usim.k, args.nas.tmsg.attacker_usim.k);
    // printf("K: %s :: %02x\n", args.usim.k.c_str(), args.nas.tmsg.attacker_usim.k[0]);
  } else {
    srsran::console("Invalid length for K: %zu should be %d\n", args.usim.k.length(), 32);
  }
  
  if (args.usim.using_op) {
    if (32 == args.usim.op.length()) {
      ue_str_to_hex(args.usim.op, args.nas.tmsg.attacker_usim.op);
      compute_opc(nas.tmsg.attacker_usim.k, args.nas.tmsg.attacker_usim.op, args.nas.tmsg.attacker_usim.opc);
      // printf("OP: %s\n", args.usim.op.c_str());
    } else {
      srsran::console("Invalid length for OP: %zu should be %d\n", args.usim.op.length(), 32);
    }
  } else {
    if (32 == args.usim.opc.length()) {
      ue_str_to_hex(args.usim.opc, args.nas.tmsg.attacker_usim.opc);
      // printf("OPC: %s :: %02x\n", args.usim.opc.c_str(), args.nas.tmsg.attacker_usim.opc[0]);
    } else {
      srsran::console("Invalid length for OPc: %zu should be %d\n", args.usim.opc.length(), 32);
    }
  }
...
//str_to_hex
void ue_stack_lte::ue_str_to_hex(std::string str, uint8_t* hex)
{
  uint32_t    i;
...
bool ue_stack_lte::switch_off()
{
  ...
  if (nas.rrc_release_flag) { // Add rrc_release_flag
    printf("[Debug] rrc_relase_flag\n");
    rrc.timer_expired(1);
  }
  return true;
}
