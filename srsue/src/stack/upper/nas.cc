...
#define MSG_SIZE 30
#define PIPENAME "/home/kwangmin/LTE_Identity_Testing/LTE_Identity_Testing/controller/named_pipe"
...
int nas::init(usim_interface_nas* usim_, rrc_interface_nas* rrc_, gw_interface_nas* gw_, const nas_args_t& cfg_)
{
  ...
  //custom value setup
  emergency_attach = cfg_.emergency_attach;
  sec_hdr_plain = cfg_.sec_hdr_plain;
  auth_fail_cause = cfg_.auth_fail_cause;
  attach_mac_zero = cfg_.attach_mac_zero;
  attach_plain = cfg_.attach_plain;
  keep_attach = cfg_.keep_attach;
  verify_sr = cfg_.verify_sr;
  tmsg = cfg_.tmsg;
  srsran::console("Test File Name: %s\n", tmsg.tfile_args.tfile_name.c_str());
  
  //Init testing parameter
  testing_mode = cfg_.testing_mode;
  testing_role = cfg_.testing_role;
  testing_state = cfg_.testing_state;
  
  // Reset the testing flag to match the testing role
  if (testing_mode == "multi_usrp"){
    if (testing_role == "attacker"){
      testing = true;
    }
  }

  // Set "detach_request" flag for initial attach
  if (verify_sr) {
    root_first_detach = false;
  } 
  else {
    root_first_detach = true;
  }

  test::init(usim_, rrc_, gw_);
  if (parse_test_list(tmsg) != SRSRAN_SUCCESS) {
    srsran::console("[error] test list file parsing error\n");
  }


  //EEA EIA value setup
  if (parse_security_algorithm_list(cfg_.eia, origin_eia_caps) != SRSRAN_SUCCESS) {
    logger.warning("Failed to parse integrity protection algorithm list: Defaulting to EIA1-128, EIA2-128, EIA3-128");
    origin_eia_caps[0] = false;
    origin_eia_caps[1] = true;
    origin_eia_caps[2] = true;
    origin_eia_caps[3] = true;
  }

  // parse and sanity check EEA list
  if (parse_security_algorithm_list(cfg_.eea, origin_eea_caps) != SRSRAN_SUCCESS) {
    logger.warning("Failed to parse encryption algorithm list: Defaulting to EEA0, EEA1-128, EEA2-128, EEA3-128");
    origin_eea_caps[0] = true;
    origin_eea_caps[1] = true;
    origin_eea_caps[2] = true;
    origin_eea_caps[3] = true;
  }
...
void nas::run_tti()
{
  ...
  if (check_no_response){
    ...
    if (elapsed_time >= 4 && testing) {
      start_time = std::chrono::steady_clock::now();
      logging_dl_message(tmsg.logfile.c_str(), "No Response [System Detection (Testing Procedure)]");
      rrc_release_flag = true;
      raise(SIGUSR2);
    }
    else if (elapsed_time >= 10 && verify) {
      start_time = std::chrono::steady_clock::now();
      printf("Verify No Response\n");
      raise(SIGUSR2);
    }
  }
...
{
  srsran::console("Timer expired (ignore)\n");
  logging_dl_message(tmsg.logfile.c_str(), "No Response");
  expire_flag = true;
  rrc_release_flag = true;
  std::cout << "timer_expired; Flag:" << testing << "++++++" << verify << std::endl;
  if (testing)
  {
    if (t3410.is_running()) {
  ...
  else { // Timer expire during initial attach
    if (t3410.is_running()) {
      logger.debug("Stopping T3410");
      t3410.stop();
    }    
...
{
  if (!testing && !verify) {
    first_detach = root_first_detach;
  }
  if (!testing && !verify){
    std::ifstream fileCheck(tmsg.logfile);
    if (fileCheck.good()){
      fileCheck.close();
      if(std::remove(tmsg.logfile.c_str()) != 0) {
        std::cerr << "Error deleting file" << std::endl;
      }
    }
  }

  // Write the progress of log file
  if (!testing && !verify){
    std::string string1 = tmsg.tfile_args.tfile_name.c_str();
    std::string string2 = tmsg.logfile.c_str();
    size_t start_pos = string1.find_last_of("/") + 1;
    size_t end_pos = string1.find(".conf");
    std::string extracted_value = string1.substr(start_pos, end_pos - start_pos);

    size_t last_slash_pos = string2.find_last_of("/");
    std::string base_path = string2.substr(0, last_slash_pos + 1);
    std::string output_file = base_path + "execution_list/" + extracted_value + ".txt";
    std::string log_file_name = string2.substr(last_slash_pos + 1);
    std::ofstream outfile(output_file, std::ios::app);
    if (outfile.is_open()) {
      outfile << log_file_name << std::endl;
      outfile.close();
...
bool nas::switch_off()
{
  ...
  {
    if (!rrc_release_flag) {
      srsran::console("[error] rrc release flag (false) error!\n");
      return false;
    }
    enter_emm_null();
  }
  else if (testing && !keep_attach)
  {
    if (!rrc_release_flag) {
      srsran::console("[error] rrc release flag (false) error!\n");
      return false;
  ...
  else if (testing && !attach_success)
  {
    enter_emm_null();
  }
  else if (!first_detach && !testing && !verify)
  {
    if (!rrc_release_flag) {
      srsran::console("[error] rrc release flag (false) error!\n");
      return false;
  ...
  {
    if (rrc_release_flag) {
      srsran::console("[DEBUG] Don't send Detach Request\n");
    }
    else {
      detach_request(true);
    }
  ...
  if (!testing && ctxt_save)
  {
    write_ctxt_file(ctxt, ctxt_base);
    ctxt_save = false;
  }
  if (testing_mode == "single_usrp"){
    if (testing)
    {
      testing = false;
      verify = true;
      check_no_response = false; // After testing procedure, set "check_no_reponse" to false
    }
    else if (verify)
    {
      testing = false;
      verify = false;
      check_no_response = false; // Set "check_no_response" to false, because we already set "check_no_response" to true in "connection_request_completed" function
    }
    else
    {
      testing = true;
  ...
  // Reset testing flag to match testing role
  if (testing_mode == "multi_usrp"){
    if (testing_role == "attacker"){
      testing = true;
      verify = false;
    ...
    else if (testing_role == "victim"){
      printf("[switch_off] testing_role: victim\n");
      if (testing_state == "CONN"){
        printf("[switch_off] set flag for CONN state\n");
        testing = false;
        verify = false;
      }
      else if (!testing && !verify){ // If previous process is initial process, switch to the verify process
        printf("[switch_off] set flag for verify attach\n");
        testing = false;
        verify = true;
      }
      else if (!testing && verify){ // If previous process is verify process, switch to the initial process
        printf("[switch_off] set flag for initial attach\n");
        testing = false;
        verify = false;
      }  
    }
  ...
  rrc_release_flag = false;
  if (log_stream.is_open()){ //Check whether ofstream is open.
        printf("close log stream\n");
        log_stream.close();
  }
  
    
  srsran::console("------------------ switch off successful ------------------\n");
  return true;
}

bool nas::set_test_config()
{
  printf("testing mode: %s, verify: %d\n", testing_mode.c_str(), verify);
  if (((testing_mode == "single_usrp") && (testing || verify)) || ((testing_mode == "multi_usrp") && verify)) // If "multi_usrp" mode, should pass this condition for executing "parse_msg_list"
    return true; 
  srsran::console("Test Message Parsing Start\n");
  ctxt_save = true;
  ctxt_read = true;
  int ret_value = parse_msg_list();
  if (ret_value == SRSRAN_ERROR) {
    srsran::console("[error] Test list file parsing error\n");
    return false;
  }
  else if (ret_value == SRSRAN_ERROR_OUT_OF_BOUNDS)
  {
    srsran::console("[debug] Finish all test\n");
    return false;
  }
...
void nas::start_attach_request(srsran::establishment_cause_t cause_)
{
  ...
  srsran::console("Attach Request with cause %s.\n", to_string(cause_).c_str());
  ...
  // start T3410
  // if t3410 is running, should be stop regardless which procedure
  if (t3410.is_running()) {
    logger.debug("Stopping T3410");
    t3410.stop();
  }
  ...
  if (verify && verify_sr) {
    srsran::console("Go to \"start_service_request\"\n");
    state.set_registered(emm_state_t::registered_substate_t::normal_service);
    start_service_request(srsran::establishment_cause_t::mo_data);
    return;
  }
  ...
  {
    logging_msg = "******************* Initial Attach *******************";
  }
  logging_message(tmsg.logfile.c_str(), logging_msg);

  gen_attach_request(msg);
  if (rrc->is_connected()) {
    srsran::console("[debug] RRC Connected\n");
    rrc->write_sdu(std::move(msg));
  } else {
    logger.debug("Initiating RRC Connection");
    srsran::console("[debug] Initiating RRC Connection\n");
...
void nas::start_service_request(srsran::establishment_cause_t cause_)
{
  ...
  if (rrc->is_connected()) {
    ...
    srsran::console("[ERROR] NAS is already registered and RRC connected.\n");
...
void nas::start_tau_request(srsran::establishment_cause_t cause_)
{
  logger.info("TAU Request with cause %s.", to_string(cause_).c_str());
  srsran::console("TAU Request with cause %s.\n", to_string(cause_).c_str());
  if (rrc->is_connected()) {
    logger.info("NAS is already registered and RRC connected");
    srsran::console("[ERROR] NAS is already registered and RRC connected.\n");
    logger.info("TAU request ignored. State = %s", state.get_full_state_text().c_str());
    return;
  }
  logger.info("NAS is already registered but RRC disconnected. Connecting now...");
  std::string logging_msg;
...
bool nas::detach_request(const bool switch_off)
{
  ...
  if (switch_off) {
    srsran::console("[DEBUG] <%s> switch_off\n", __func__);
    enter_emm_null();
  }
  return false;
}
...
{
  if (verify) {
    printf("Start to check expire time\n");
    start_time = std::chrono::steady_clock::now(); // Set current start time
  ...
  if (outcome == true) {
    ...
    if (verify)
    {
      // logging_dl_message(tmsg.logfile.c_str(), "Service Successful", "DEBUG");
    if (t3410.is_running()) {
      logger.debug("Stopping T3410");
      t3410.stop();
    }
      printf("connection_request_completed[verify]\n");      
    }
  } else {
...
{
  if (nof_plmns == -999) { // Receive RRC Reconf complete after service request
    if (verify && verify_sr){
      printf("[debug] Service Request Successfully Finish");
      logging_dl_message(tmsg.logfile.c_str(), "[RRC] Service Request Successfully");
      raise(SIGUSR2);
    }
  }
...
{
  srsran::console("[DEBUG] [%s] RRC no longer connected.\n",testing_role.c_str());
  logger.debug("RRC no longer connected. NAS State %s.", state.get_full_state_text().c_str());
  if (testing_role == "victim")
    logging_dl_message(tmsg.logfile.c_str(), "[RRC] Victim RRC no longer connected");
  else if (testing_role == "attacker")
    logging_dl_message(tmsg.logfile.c_str(), "[RRC] Attacker RRC no longer connected");
  if (testing_state == "CONN" && testing_role == "victim"){
    std::string tmpfilename = "/tmp/ping_test.txt";
    std::ofstream outfile(tmpfilename);
    if (!outfile) {
        std::cerr << "Error: Could not open file for writing\n";
        return;
    }
    std::string content = "Victim RRC no longer connected";
    outfile << content;
    outfile.close();
  }
  // For detecting RRC release during test procedure in victim session
  if (!testing && !verify){
    if (t3410.is_running()) {
      logger.debug("Stopping T3410");
      t3410.stop();
    }
    printf("[left_rrc_connected] Raise SIGUSR1\n");    
    raise(SIGUSR1);
  }
  else if (verify && verify_sr){
    if (t3410.is_running()) {
      logger.debug("Stopping T3410");
      t3410.stop();
    }    
    printf("[left_rrc_connected] Raise SIGUSR2\n");
    raise(SIGUSR2);
  }
}
...
void nas::write_pdu(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  switch (sec_hdr_type) {
    ...
    case LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED:
      printf("[write_pdu] Integrity and ciphered or Integrity\n");
      if ((integrity_check(pdu.get()))) {
        if (sec_hdr_type == LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED) {
          printf("[write_pdu] Integrity and ciphered\n");
          cipher_decrypt(pdu.get());
        }
        break;
      } else {
        logger.error("Not handling NAS message with integrity check error");
        srsran::console("Not handling NAS message with integrity check error. But keep processing\n");
        printf("[write_pdu] integrity_ckeck ERROR\n");
        break;
      }
    default:
  ...
  switch (msg_type) {
    case LIBLTE_MME_MSG_TYPE_ATTACH_ACCEPT:
      parse_attach_accept(lcid, std::move(pdu), sec_hdr_type);
      break;
    case LIBLTE_MME_MSG_TYPE_ATTACH_REJECT:
    ...
    case LIBLTE_MME_MSG_TYPE_CLOSE_UE_TEST_LOOP:
      ...
      break;
    case LIBLTE_MME_MSG_TYPE_TRACKING_AREA_UPDATE_REJECT:
      parse_tracking_area_update_reject();
      break;
...
//For logging parse function
void nas::logging_dl_message(std::string file, std::string message, std::string notes)
{
  std::string full_msg;
  if (notes.empty())
  {
    full_msg = "[UE <- NAS] " + message;
  }
  else
  {
    full_msg = "[UE <- NAS] <" + notes + "> " + message;
  }
  logging_message(file, full_msg);
}

void nas::send_end_indicator(std::string msg)
{
  int fd, nread;
  printf("send_end_indicator\n");
  if((fd = open(PIPENAME, O_WRONLY)) < 0){
    printf("fail to open named pipe\n");
    return;
  }

  if((nread = write(fd, msg.c_str(), sizeof(msg.c_str()))) < 0) {
...
void nas::parse_attach_accept(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  logging_dl_message(tmsg.logfile.c_str(), "Attach Accept");
  ...
  // Don't stop T3410 if it's testing process. For logging message after "attach accept". (If t3410 is expired, we can't log "No Response" situation.)
  // stop T3410
  if (t3410.is_running() && !testing){
    printf("[parse_attach_accept] Stop t3410\n");
    logger.debug("Stopping T3410");
    t3410.stop();
  }
  else{
    printf("[parse_attach_accept] Don't stop t3410\n");
  }

  // Change current_sec_hdr
  ...
  if (true) {
    ...
    if (attach_accept.guti_present) {
      memcpy(&tmsg.origin_guti, &ctxt.guti, sizeof(LIBLTE_MME_EPS_MOBILE_ID_GUTI_STRUCT));
      ...
      //print GUTI
      srsran::console("Received GUTI -----------------------------\n");
      srsran::console("MMEGID: %u\n", ctxt.guti.mme_group_id);
      srsran::console("MMEC: %u\n", s_tmsi.mmec);
      srsran::console("TMSI: %x\n", s_tmsi.m_tmsi);
      srsran::console("Received GUTI -----------------------------\n");
      std::ofstream ofs;
      log_stream << "            --> With Sec_Hdr_Type (" << (unsigned)sec_hdr_type << ")" << std::endl;
      log_stream << "            --> With GUTI (0x" << std::hex << s_tmsi.m_tmsi << ")" << std::endl;

      if (!test_guti_save && testing)
      {
        memcpy(&ctxt.guti, &tmsg.origin_guti, sizeof(LIBLTE_MME_EPS_MOBILE_ID_GUTI_STRUCT));
      }
    }
    ...
    if (eps_bearer.insert(eps_bearer_map_pair_t(bearer.eps_bearer_id, bearer)).second) {
      ...
      ctxt_base.rx_count++; //increase rx_count before sending "attach complete" (For logging)
      //save in tmsg structure
      tmsg.bearer_id = bearer.eps_bearer_id;
      tmsg.transaction_id = transaction_id;
      if (!testing || verify) {
        send_attach_complete(transaction_id, bearer.eps_bearer_id);
      }
      else if (check_test_num())
      {
        srsran::console("Go to \'control_test_message\' function\n");
        if (!control_test_message())
        {
          srsran::console("Run origin procedure\n");
          send_attach_complete(transaction_id, bearer.eps_bearer_id);
        }
      }
      else if (!keep_attach)
      ...
      srsran::console("[error] Error adding EPS bearer.\n");
    }

  } else {
    logger.info("Not handling attach type %u", attach_accept.eps_attach_result);
    srsran::console("[error] Not handling attach type %u\n", attach_accept.eps_attach_result);
    enter_emm_deregistered(emm_state_t::deregistered_substate_t::plmn_search);
  }
...
void nas::parse_attach_reject(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  char buf[30];
  sprintf(buf, "Attach Reject(Cause=%02X)", attach_rej.emm_cause);
  logging_dl_message(tmsg.logfile.c_str(), buf);
  log_stream << "            --> With Sec_Hdr_Type (" << (unsigned)sec_hdr_type << ")" << std::endl;
  std::cout << testing << "++++1++" << verify << std::endl;
  rrc_release_flag = true;
  if (testing)
  {
    if (t3410.is_running()) {
...
void nas::parse_authentication_request(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Authentication Request");

  logger.info("Received Authentication Request");
  liblte_mme_unpack_authentication_request_msg((LIBLTE_BYTE_MSG_STRUCT*)pdu.get(), &auth_req);

  log_stream << "            --> With KSI (" << (unsigned)auth_req.nas_ksi.nas_ksi << ")" << std::endl;
  ...
  if (auth_fail_cause != 0)
  {
    send_authentication_failure(auth_fail_cause, nullptr);
    return;
  }

  memcpy(tmsg.origin_k_asme, ctxt.k_asme, 32);
  auth_result_t auth_result;
  if (testing && !using_victim_key)
  {
    auth_result = attacker_generate_authentication_response(auth_req.rand, auth_req.autn, mcc, mnc, res, &res_len, ctxt.k_asme);
  }
  else
  {
    auth_result = usim->generate_authentication_response(auth_req.rand, auth_req.autn, mcc, mnc, res, &res_len, ctxt.k_asme);
  }
  
  std::stringstream ss_k_asme;
  ss_k_asme << std::hex << std::setfill('0');
  for(int i=0; i < 32; i++) ss_k_asme << std::setw(2) << std::setfill('0') << (int)ctxt.k_asme[i];
  std::cout << "[Auth. Req] K_ASME: " << std::hex << ss_k_asme.str() << std::endl;
  std::cout << "[Auth. Req] auth req KSI: " << (int)auth_req.nas_ksi.nas_ksi << std::endl;

  if (!auth_ksi_fresh && testing)
  {
    std::cout << "[Auth. Req] Change K from origin_k" << std::endl;
    memcpy(ctxt.k_asme, tmsg.origin_k_asme, 32);
  }

  logger.debug(res, res_len, "Authentication request RES");

  srsran::console("[Auth. Req] res_len: %d\n", res_len);
  std::cout << "[Auth. Req] RES: " << std::hex << +res[0] << +res[1] << +res[2] << +res[3] << +res[4] << +res[5] << +res[6] << +res[7] << std::endl;

  std::stringstream ss_rand;
  std::stringstream ss_autn;
  ss_rand << std::hex << std::setfill('0');
  ss_autn << std::hex << std::setfill('0');
  for(int i=0; i < 16; i++) ss_rand << std::setw(2) << std::setfill('0') << (int)auth_req.rand[i];
  for(int i=0; i < 16; i++) ss_autn << std::setw(2) << std::setfill('0') << (int)auth_req.autn[i];
  std::cout << "[Auth. Req] RAND: " << std::hex << ss_rand.str() << std::endl;
  std::cout << "[Auth. Req] AUTN: " << std::hex << ss_autn.str() << std::endl;
  ...
  std::cout << "[Auth. Req] ctxt KSI: " << (int)ctxt.ksi << std::endl;

  uint8_t origin_cause = LIBLTE_MME_EMM_CAUSE_MAC_FAILURE;
  if (auth_result == AUTH_OK) {
    ...
    origin_cause = LIBLTE_MME_EMM_CAUSE_MAC_FAILURE;
    // send_authentication_failure(LIBLTE_MME_EMM_CAUSE_MAC_FAILURE, nullptr);
  }
  // check config file
  memcpy(tmsg.auth_res, res, 16);
  tmsg.auth_res_len = res_len;
  if (!testing || verify) {
    if (auth_request)
    {
    ...
    {
      send_authentication_failure(origin_cause, res);
    }
  }
  else if (check_test_num())
  {
...
{
  logging_dl_message(tmsg.logfile.c_str(), "Authentication Reject");
  rrc_release_flag = true;
...
void nas::parse_identity_request(unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  std::string message = "Identity Request";
  switch (id_req.id_type)
  {
    case 0x1:
      ...
      break;
    case 0x2:
      message = message + " (IMEI)";
      break;
    case 0x3:
      message = message + " (IMEISV)";
      break;
    case 0x4:
      message = message + " (TMSI)";
      break;
    case 0x5:
      message = message + " (TMGI)";
      break;
    default:
      printf("[error] Parse Identity Request ID Type Error\n");
  }
  logging_dl_message(tmsg.logfile.c_str(), message);
  log_stream << "            --> With Sec_Hdr_Type (" << (unsigned)sec_hdr_type << ")" << std::endl;

  logger.info("Received Identity Request. ID type: %d", id_req.id_type);
  ctxt_base.rx_count++;

  tmsg.id_type = id_req.id_type;
  if (!testing || verify) {
    if (sec_hdr_type >= LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY ||
...
{
  logging_dl_message(tmsg.logfile.c_str(), "Security Mode Command");
  tmsg.reject_flag = false;
  ...
  log_stream << "            --> With KSI (" << (unsigned)sec_mode_cmd.nas_ksi.nas_ksi << ")" << std::endl;
  log_stream << "            --> With EEA (" << ciphering_algorithm_id_text[sec_mode_cmd.selected_nas_sec_algs.type_of_eea] << ")" << std::endl;
  log_stream << "            --> With EIA (" << integrity_algorithm_id_text[sec_mode_cmd.selected_nas_sec_algs.type_of_eia] << ")" << std::endl;
  log_stream << "            --> With IMEISV_req (" << sec_mode_cmd.imeisv_req_present << ")" << std::endl;
  ...
  srsran::console("Received Security Mode Command ksi: %d, eea: %s, eia: %s\n",
              sec_mode_cmd.nas_ksi.nas_ksi,
              ciphering_algorithm_id_text[sec_mode_cmd.selected_nas_sec_algs.type_of_eea],
              integrity_algorithm_id_text[sec_mode_cmd.selected_nas_sec_algs.type_of_eia]);
  ...
  if (have_ctxt) {
    if (sec_mode_cmd.nas_ksi.nas_ksi != ctxt.ksi) {
      ...
      tmsg.smc_reject_cause = LIBLTE_MME_EMM_CAUSE_SECURITY_MODE_REJECTED_UNSPECIFIED;
      tmsg.reject_flag = true;
    }
  }
  ...
  if (!check_cap_replay(&sec_mode_cmd.ue_security_cap)) {
    ...
    tmsg.smc_reject_cause = LIBLTE_MME_EMM_CAUSE_UE_SECURITY_CAPABILITIES_MISMATCH;
    tmsg.reject_flag = true;
  }
  ...
  std::stringstream ss_k_nas_int;
  ss_k_nas_int << std::hex << std::setfill('0');
  for(int i=0; i < 32; i++) ss_k_nas_int << std::setw(2) << std::setfill('0') << (int)ctxt_base.k_nas_int[i];
  std::cout << "[SMC] K_NAS_int: " << std::hex << ss_k_nas_int.str() << std::endl;

  std::cout << "[SMC] rx_count: " << (uint)ctxt_base.rx_count << std::endl;
  ...
  if (not integrity_check(pdu.get())) {
    ...
    tmsg.smc_reject_cause = LIBLTE_MME_EMM_CAUSE_MAC_FAILURE;
    tmsg.reject_flag = true;
  }

  ctxt_base.rx_count++;

  bool pri_imeisv_present;
  if (sec_mode_cmd.imeisv_req_present && LIBLTE_MME_IMEISV_REQUESTED == sec_mode_cmd.imeisv_req) {
    pri_imeisv_present    = true;
  } else {
    pri_imeisv_present = false;
  }
  tmsg.imeisv_present = pri_imeisv_present;
  ...
  //Security mode command Check
  if (!testing || verify) {
    if (tmsg.reject_flag)
    {
      send_security_mode_reject(tmsg.smc_reject_cause);
    }
    else
    {
      send_security_mode_complete(pri_imeisv_present);
    }
  }
  else if (check_test_num())
  {
    printf("This should not be printed\n");
    srsran::console("Go to \'control_test_message\'\n");
    if (!control_test_message())
    {
      srsran::console("Run origin procedure\n");
      if (tmsg.reject_flag)
...
void nas::parse_tracking_area_update_reject()
{
  logging_dl_message(tmsg.logfile.c_str(), "Tracking Area Update Reject");
  raise(SIGUSR2);
  return;
}
...
void nas::parse_service_reject(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  char buf[30];
  sprintf(buf, "Service Reject(Cause=0x%x)", service_reject.emm_cause);
  logging_dl_message(tmsg.logfile.c_str(), buf);
  log_stream << "            --> With Sec_Hdr_Type (" << (unsigned)sec_hdr_type << ")" << std::endl;
  ...
  logging_ul_message(tmsg.logfile.c_str(), "Attach Request");
...
void nas::parse_esm_information_request(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "ESM Information Request");
...
void nas::parse_emm_information(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "EMM information");
    ...
    if (check_test_num())
      ...
      if (!control_test_message())
      {
        ...
        t3410.stop();
          ...
          logger.info("parse_emm_information debug\n");
        }
      }
      else if (t3410.is_running()) {
        logger.debug("Stopping T3410");
        t3410.stop();
      }    
  }
  ...
  {
    if (!verify_sr || (testing_mode == "multi_usrp" && testing_state == "CONN")) {// if using "Service Request" to verify, raise SIGUSR1 when expiring rrc inactivity timer
      if (t3410.is_running()) {
        logger.debug("Stopping T3410");
        t3410.stop();
      }    
...
void nas::parse_detach_request(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Detach Request");
...
void nas::parse_activate_dedicated_eps_bearer_context_request(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Activate Dedicated EPS Bearer Context Request");
...
void nas::parse_deactivate_eps_bearer_context_request(unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Deactivate EPS Bearer Context Request");
...
void nas::parse_modify_eps_bearer_context_request(srsran::unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Modify EPS Bearer Context Request");
...
void nas::parse_activate_test_mode(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Activate Test Mode");
...
void nas::parse_close_ue_test_loop(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "Close UE Test Loop");
...
void nas::parse_emm_status(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  logging_dl_message(tmsg.logfile.c_str(), "EMM Status");
 ...
 * Senders
 ******************************************************************************/
void nas::logging_ul_message(std::string file, std::string message, std::string notes)
{
  std::string full_msg;
  if (notes.empty())
  {
...
void nas::gen_attach_request(srsran::unique_byte_buffer_t& msg)
{
  ...
  {
    ctxt_base.cipher_algo = CIPHERING_ALGORITHM_ID_EEA0;
    ctxt_base.integ_algo = INTEGRITY_ALGORITHM_ID_EIA0;
  }
  if (emergency_attach)
  {
  ...
  {
    // initial & verify attach
    attach_req.eps_attach_type = LIBLTE_MME_EPS_ATTACH_TYPE_COMBINED_EPS_IMSI_ATTACH;
  }

  for (u_int32_t i = 0; i < 8; i++) {
    attach_req.ue_network_cap.eea[i] = origin_eea_caps[i];
    attach_req.ue_network_cap.eia[i] = origin_eia_caps[i];
  }
  ...
  if (have_guti && have_ctxt) {
    ...
    if (attach_plain)
    {
      temp_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
    }
    else
    {
      temp_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY;
    }    
  } else {
  ...
  if (testing && !keep_attach) {
    rrc_release_flag = true;
  }
  else if(!first_detach) {
    rrc_release_flag = true;
  } 
  uint32_t ctxt_sqn;
  bool test_gen_flag = false;
  //gen test attach request
  if (!testing && !verify) {
    // Initial attach msg
    attach_req.eps_mobile_id.type_of_id = LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI;
    attach_req.nas_ksi.tsc_flag         = LIBLTE_MME_TYPE_OF_SECURITY_CONTEXT_FLAG_NATIVE;
    ...
    usim->get_imsi_vec(attach_req.eps_mobile_id.imsi, 15);
    logger.info("Requesting IMSI attach (IMSI=%s)", usim->get_imsi_str().c_str());
    srsran::console("Requesting IMSI attach (IMSI=%s)\n", usim->get_imsi_str().c_str());
    temp_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
    gen_pdn_connectivity_request(&attach_req.esm_msg);
    liblte_mme_pack_attach_request_msg(&attach_req, temp_sec_hdr, 0, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    logging_ul_message(tmsg.logfile.c_str(), "Attach Request");
    std::ofstream ofs;
    ofs.open(tmsg.logfile.c_str(), std::ios_base::app);
    ofs << "            --> With IMSI (" << usim->get_imsi_str().c_str() << ")" << std::endl;
    ofs.close();
  }
  else if (verify) {
    gen_pdn_connectivity_request(&attach_req.esm_msg);
    liblte_mme_pack_attach_request_msg(
        &attach_req, temp_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());

    if (apply_security_config(msg, temp_sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
    }
    logging_ul_message(tmsg.logfile.c_str(), "Attach Request");
    log_stream << "            --> With GUTI (0x" << std::hex << ctxt.guti.m_tmsi << ")" << std::endl;
    log_stream << "            --> With KSI (" << (unsigned)attach_req.nas_ksi.nas_ksi << ")" << std::endl;
    log_stream << "            --> With SQN (" << (unsigned)ctxt_base.tx_count << ")" << std::endl;
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (first_attach)
    {
      first_attach = false;
    }
    if (gen_test_attach_request(attach_req, ctxt, have_guti, have_ctxt) != SRSRAN_SUCCESS) {
      srsran::console("Attach request generation fail\n");
      return;
    }
    //logging Attach Request Message
    if (tmsg.conf_map["sqn"].compare("invalid") == 0){ // Set tx_count to 0, if "sqn" value is "invalid" in testing "Attach Request"
      ctxt_base.tx_count = 0; 
    }
    ...
    if (parse_hdr_and_mac() != SRSRAN_SUCCESS) {
      srsran::console("\'parse_hdr_and_mac\' ERROR!\n");
      return;
    }
    srsran::console("[debug] sec_hdr: %x, have_guti: %d, have_ctxt: %d\n", tmsg.sec_hdr, have_guti, have_ctxt);
    test_gen_flag = true;

    if (tmsg.sec_hdr == LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      liblte_mme_pack_attach_request_msg(&attach_req, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    }
    else {
      if (have_guti && have_ctxt) {
        srsran::console("[debug] have_guti, have_ctxt\n");
        // Set tx_count to valid/invalid
        if (tmsg.conf_map["sqn"].compare("invalid") == 0) {
          printf("Set invalid tx_count\n");
          ctxt_sqn = 0;
        }
        else {
          ctxt_sqn = ctxt_base.tx_count;
        }
        liblte_mme_pack_attach_request_msg(&attach_req, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());

        if (apply_security_config(msg, tmsg.sec_hdr)) {
          logger.error("Error applying NAS security.");
          return;
        }
      }
      else {
        liblte_mme_pack_attach_request_msg(&attach_req, tmsg.sec_hdr, 0, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
      }
    }

    if (set_mac_value((LIBLTE_BYTE_MSG_STRUCT*)msg.get()) != SRSRAN_SUCCESS) {
      srsran::console("\"set_mac_value\" ERROR !\n");
      return;
    }
    logging_ul_message(tmsg.logfile.c_str(), "Attach Request", "TEST");
    log_stream << "            --> With KSI (" << (unsigned)attach_req.nas_ksi.nas_ksi << ")" << std::endl;
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    temp_sec_hdr = tmsg.sec_hdr;
  }
  else {
    srsran::console("There is no config file for attach request\n");
    gen_pdn_connectivity_request(&attach_req.esm_msg);
    if (have_guti && have_ctxt) {
      liblte_mme_pack_attach_request_msg(&attach_req, temp_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
      if (apply_security_config(msg, temp_sec_hdr)) {
        logger.error("Error applying NAS security.");
        return;
      }
  ...
  if (temp_sec_hdr >= LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY){
    if (test_gen_flag) 
      log_stream << "            --> Sequence number (TX: "<< ctxt_sqn <<", RX: " << ctxt_base.rx_count << " )"<< std::endl;
    else
      log_stream << "            --> Sequence number (TX: "<< ctxt_base.tx_count <<", RX: " << ctxt_base.rx_count << " )"<< std::endl;
  }
  log_stream << "            --> Cipher algorithm: " << ctxt_base.cipher_algo << " / Integrity algorithm: " << ctxt_base.integ_algo << std::endl;
  ...
  if (have_ctxt) {
    ...
    if (temp_sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS)
    {
      ctxt_base.tx_count++;
    }
  }
  ...
  for (u_int32_t i = 0; i < 8; i++) {
    eea_caps[i] = attach_req.ue_network_cap.eea[i];
    eia_caps[i] = attach_req.ue_network_cap.eia[i];
  }
...
void nas::gen_service_request(srsran::unique_byte_buffer_t& msg)
{
  ...
  if (!testing || verify) {
    ...
    msg->msg[3] = mac[3];
    msg->N_bytes++;
    std::cout << "mac[2]: 0x" << std::hex << static_cast<int>(mac[2]) 
              << ", mac[3]: 0x" << std::hex << static_cast<int>(mac[3]) 
              << std::dec << std::endl;
    logging_ul_message(tmsg.logfile.c_str(), "Service Request");
    log_stream << "            --> With GUTI (0x" << std::hex << ctxt.guti.m_tmsi << ")" << std::endl;
    set_k_enb_count(ctxt_base.tx_count);
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_service_request(service_req, ctxt, ctxt_base.tx_count) != SRSRAN_SUCCESS) {
      srsran::console("Service request generation fail\n");
      return;
    }
    test_gen_flag = true;
    if (parse_hdr_and_mac() != SRSRAN_SUCCESS) {
    ...
    srsran::console("[debug] sec_hdr: %x, have_guti: %d, have_ctxt: %d\n", tmsg.sec_hdr, have_guti, have_ctxt);
    // Set tx_count to invalid
    if (tmsg.conf_map["sqn"].compare("invalid") == 0) {
      printf("Set tx_count to invalid\n");
      ctxt_sqn = 0;
    }
    else {
      ctxt_sqn = ctxt_base.tx_count;
    ...
    liblte_mme_pack_service_request_msg(&service_req, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    if (tmsg.conf_map["short_mac"].compare("valid") == 0){
      uint8_t mac[4];
      integrity_generate(&ctxt_base.k_nas_int[16], ctxt_base.tx_count, SECURITY_DIRECTION_UPLINK, &msg->msg[0], 2, &mac[0]);
      service_req.short_mac = (static_cast<uint16_t>(mac[2]) << 8) | mac[3];
      printf("[debug] Valid short MAC\n");
      std::cout << "short_mac: 0x" << std::hex << service_req.short_mac << std::dec << std::endl;
      std::cout << "mac[2]: 0x" << std::hex << static_cast<int>(mac[2]) 
              << ", mac[3]: 0x" << std::hex << static_cast<int>(mac[3]) 
              << std::dec << std::endl;
      liblte_mme_pack_service_request_msg(&service_req, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    }
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST) {
      if (apply_security_config(msg, tmsg.sec_hdr)) {
        logger.error("Error applying NAS security.");
        return;
      }
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Service Request", "TEST");
  }
  else {
    srsran::console("There is no config file for service request\n");
    // Pack the service request message directly
    msg->msg[0] = (LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST << 4u) | (LIBLTE_MME_PD_EPS_MOBILITY_MANAGEMENT);
    msg->N_bytes++;
    msg->msg[1] = (ctxt.ksi & 0x07u) << 5u;
...
void nas::gen_tau_request(srsran::unique_byte_buffer_t& msg)
{
  if (testing) {
    start_time = std::chrono::steady_clock::now(); // Set current start time
    check_no_response = true;
  ...
  srsran::console("Gen TAU Request\n");
  logger.info("Generating TAU request");
  ...
  uint32_t ctxt_sqn;
  if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_tau_request(tau_req, ctxt) != SRSRAN_SUCCESS) {
      srsran::console("TAU request generation fail\n");
      return;
    }
    if (parse_hdr_and_mac() != SRSRAN_SUCCESS) {
      srsran::console("\'parse_hdr_and_mac\' ERROR!\n");
    ...
    liblte_mme_pack_tracking_area_update_request_msg(&tau_req, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST) {
      if (apply_security_config(msg, tmsg.sec_hdr)) {
        logger.error("Error applying NAS security.");
        return;
    ...
    logging_ul_message(tmsg.logfile.c_str(), "TAU Request", "TEST");
  }
  log_stream << "            --> Sequence number (TX: "<< ctxt_sqn <<", RX: " << ctxt_base.rx_count << " )"<< std::endl;
...
void nas::gen_pdn_connectivity_request(LIBLTE_BYTE_MSG_STRUCT* msg)
{
  ...
  //test pdn_connectivity_request
  if (testing)
  {
    if (gen_test_pdn_connectivity_request(pdn_con_req) != SRSRAN_SUCCESS) {
      srsran::console("PDN connectivity request generation fail\n");
    }
  }
...
void nas::send_security_mode_reject(uint8_t cause)
{
  ...
  if (!testing || verify) {
    srsran::console("[Initial attach] Security mode reject\n");
    liblte_mme_pack_security_mode_reject_msg(&sec_mode_rej, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (pcap != nullptr) {
      pcap->write_nas(pdu->msg, pdu->N_bytes);
    }
    logger.info("Sending security mode reject");
    logging_ul_message(tmsg.logfile.c_str(), "Security Mode Reject");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_sec_mode_reject(sec_mode_rej) != SRSRAN_SUCCESS) {
      srsran::console("Security mode reject generation fail\n");
      return;
    }
    ...
    if (tmsg.sec_hdr == LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      ...
      if (have_guti && have_ctxt) {
        ...
        liblte_mme_pack_security_mode_reject_msg(&sec_mode_rej, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
        ctxt_base.tx_count++;
      }
      else {
        liblte_mme_pack_security_mode_reject_msg(&sec_mode_rej, tmsg.sec_hdr, 0, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
      }
    }
    ...
    if (apply_security_config(pdu, tmsg.sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
    }

    if (set_mac_value((LIBLTE_BYTE_MSG_STRUCT*)pdu.get()) != SRSRAN_SUCCESS) {
      srsran::console("\"set_mac_value\" ERROR !\n");
      return;
    }
    logging_ul_message(tmsg.logfile.c_str(), "Security Mode Reject", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
  }
  else {
    srsran::console("There is no config file for security mode reject\n");
    liblte_mme_pack_security_mode_reject_msg(&sec_mode_rej, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (pcap != nullptr) {
      pcap->write_nas(pdu->msg, pdu->N_bytes);
    }
...
// Security Mode Complete transmission
void nas::send_security_mode_complete(bool imeisv_present)
{
  if (testing) {
    start_time = std::chrono::steady_clock::now(); // Set current start time
    check_no_response = true;
  ...
  uint8_t tmp_sec_hdr;
  if (!testing || verify) {
    srsran::console("[Initial attach] Security mode complete\n");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_sec_mode_complete(sec_mode_comp, ctxt) != SRSRAN_SUCCESS) {
      srsran::console("Security mode complete generation fail\n");
      return;
    }
    ...
    srsran::console("There is no config file for security mode complete\n");
  }
  ...
  if (test_gen_flag) {
    ...
    liblte_mme_pack_security_mode_complete_msg(
      &sec_mode_comp, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Security Mode Complete", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    tmp_sec_hdr = tmsg.sec_hdr;
  }
  else {
    liblte_mme_pack_security_mode_complete_msg(
    ...
    logger.info(
        "Sending Security Mode Complete ctxt_base.tx_count=%d", ctxt_base.tx_count);
    logging_ul_message(tmsg.logfile.c_str(), "Security Mode Complete");
    log_stream << "            --> With SQN (" << (unsigned)ctxt_base.tx_count << ")" << std::endl;
    tmp_sec_hdr = current_sec_hdr;
  ...
  log_stream << "            --> Cipher algorithm: " << ctxt_base.cipher_algo << " / Integrity algorithm: " << ctxt_base.integ_algo << std::endl;
  rrc->write_sdu(std::move(pdu));
  if (tmp_sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS)
  {
    ctxt_base.tx_count++;
  }
}
...
void nas::send_detach_request(bool switch_off)
{
  ...
  uint8_t tmp_sec_hdr;
  uint8_t origin_sec_hdr = current_sec_hdr;
  LIBLTE_MME_DETACH_REQUEST_MSG_STRUCT detach_request = {};
  if (switch_off) {
  ...
  if (have_guti && have_ctxt) {
    ...
    current_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY;
  } else {
  ...
  std::string logging_msg;
  if (!testing || verify) {
    srsran::console("[Initial attach] Detach request\n");
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Detach Request");
  }
  else if (testing && check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    logging_msg = "\n\n******************* Test Attach *******************";
    logging_message(tmsg.logfile.c_str(), logging_msg);
    if (testing && !keep_attach) {
      rrc_release_flag = true;
    ...
    if (gen_test_detach_request(detach_request, ctxt, have_guti, have_ctxt) != SRSRAN_SUCCESS) {
      srsran::console("Detach request generation fail\n");
      return;
    }
    ...
    if (tmsg.sec_hdr == LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      liblte_mme_pack_detach_request_msg(&detach_request, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0,(LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    }
    else {
      if (have_guti && have_ctxt) {
        srsran::console("[debug] have_guti, have_ctxt\n");
        ...
        liblte_mme_pack_detach_request_msg(&detach_request, tmsg.sec_hdr, ctxt_sqn,(LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
      }
      else {
        liblte_mme_pack_detach_request_msg(&detach_request, tmsg.sec_hdr, 0,(LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
      }
    }
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Detach Request", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    tmp_sec_hdr = tmsg.sec_hdr;
    current_sec_hdr = origin_sec_hdr;
  }
  else {
    srsran::console("There is no config file for detach request\n");
    liblte_mme_pack_detach_request_msg(
        &detach_request, current_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (pcap != nullptr) {
      pcap->write_nas(pdu->msg, pdu->N_bytes);
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Detach Request");
    if (current_sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_base.tx_count << ")" << std::endl;
    }
  }
  ...
  if (rrc->is_connected()) {
    ...
    srsran::console("[debug] RRC is not connected\n");
...
void nas::send_attach_complete(const uint8_t& transaction_id_, const uint8_t& eps_bearer_id)
{
  ...
  //make plain message
  uint8_t temp_sec_hdr = current_sec_hdr;
  if (sec_hdr_plain)
  {
    current_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
  }
  ...
  bool test_gen_flag = false;
  if (!testing || verify) {
    srsran::console("[Initial attach] Attach complete\n");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_attach_complete(act_def_eps_bearer_context_accept) != SRSRAN_SUCCESS) {
      srsran::console("Attach complete generation fail\n");
      return;
    }
    ...
    srsran::console("There is no config file for attach complete\n");
  }
  ...
  if (test_gen_flag) {
    ...
    liblte_mme_pack_attach_complete_msg(
        &attach_complete, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Attach Complete", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
  }
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Attach Complete");
    if (current_sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_base.tx_count << ")" << std::endl;
    }
  }
...
void nas::send_detach_accept()
{
  ...
  logging_ul_message(tmsg.logfile.c_str(), "Detach Accept");
  rrc->write_sdu(std::move(pdu));
}
...
void nas::send_authentication_response(const uint8_t* res, const size_t res_len)
{
  ...
  if ((tmsg.current_file.compare("pass") == 0) && run_flag)
  {
    run_flag = false;
    return;
  }
  ...
  uint8_t tmp_sec_hdr = current_sec_hdr;
  bool test_gen_flag = false;
  uint32_t ctxt_sqn;
  if (!testing || verify) {
    srsran::console("[Initial attach] Authentication response\n");
    liblte_mme_pack_authentication_response_msg(&auth_res, current_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (apply_security_config(pdu, current_sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
    }
    logging_ul_message(tmsg.logfile.c_str(), "Authentication Response");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_authentication_response(auth_res, res, res_len) != SRSRAN_SUCCESS) {
      srsran::console("Authentication response generation fail\n");
      return;
    }
    ...
    if (tmsg.sec_hdr == LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      liblte_mme_pack_authentication_response_msg(&auth_res, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    }
    else {
      if (have_guti && have_ctxt) {
        srsran::console("[debug] have_guti, have_ctxt\n");
        ...
        liblte_mme_pack_authentication_response_msg(&auth_res, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
      }
      else {
        liblte_mme_pack_authentication_response_msg(&auth_res, tmsg.sec_hdr, 0, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
      }
    }
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Authentication Response", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    tmp_sec_hdr = tmsg.sec_hdr;
    ...
    srsran::console("There is no config file for authentication response\n");
    liblte_mme_pack_authentication_response_msg(&auth_res, current_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (apply_security_config(pdu, current_sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
...
void nas::send_authentication_failure(const uint8_t cause, const uint8_t* auth_fail_param)
{
  ...
  if (cause == LIBLTE_MME_EMM_CAUSE_SYNCH_FAILURE) {
  ...
  uint8_t tmp_sec_hdr = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
  //gen test authentication failure
  uint32_t ctxt_sqn;
  if (!testing || verify) {
    srsran::console("[Initial attach] Authentication failure\n");
    printf("cause %d\n", cause);
    liblte_mme_pack_authentication_failure_msg(&auth_failure, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    logging_ul_message(tmsg.logfile.c_str(), "Authentication Failure");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_authentication_failure(auth_failure, auth_fail_param) != SRSRAN_SUCCESS) {
      srsran::console("Authentication failure generation fail\n");
      return;
    }
    ...
    if (tmsg.sec_hdr == LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      ...
      if (have_guti && have_ctxt) {
        ...
        liblte_mme_pack_authentication_failure_msg(&auth_failure, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
        ...
        liblte_mme_pack_authentication_failure_msg(&auth_failure, tmsg.sec_hdr, 0, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
      }
    }
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Authentication Failure", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    tmp_sec_hdr = tmsg.sec_hdr;
    ...
    srsran::console("There is no config file for authentication failure\n");
    liblte_mme_pack_authentication_failure_msg(&auth_failure, LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS, 0, (LIBLTE_BYTE_MSG_STRUCT*)msg.get());
    logging_ul_message(tmsg.logfile.c_str(), "Authentication Failure");
  }
...
void nas::send_identity_response(const uint8 id_type)
{
  ...
  if (!testing || verify) {
    srsran::console("[Initial attach] Identity response\n");
    liblte_mme_pack_identity_response_msg(&id_resp, current_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (apply_security_config(pdu, current_sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
    }
    logging_ul_message(tmsg.logfile.c_str(), "Identity Response");
  }
  else if (check_test_num() && (tmsg.current_file.compare("pass") != 0)) {
    if (gen_test_identity_response(id_resp, id_type, ctxt) != SRSRAN_SUCCESS) {
      srsran::console("Identity response generation fail\n");
      return;
    }
    ...
    liblte_mme_pack_identity_response_msg(&id_resp, tmsg.sec_hdr, ctxt_sqn, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    ...
    logging_ul_message(tmsg.logfile.c_str(), "Identity Response", "TEST");
    if (tmsg.sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS) {
      log_stream << "            --> With SQN (" << (unsigned)ctxt_sqn << ")" << std::endl;
    }
    temp_sec_hdr = tmsg.sec_hdr;
    ...
    srsran::console("There is no config file for identity response\n");
    liblte_mme_pack_identity_response_msg(&id_resp, current_sec_hdr, ctxt_base.tx_count, (LIBLTE_BYTE_MSG_STRUCT*)pdu.get());
    if (apply_security_config(pdu, current_sec_hdr)) {
      logger.error("Error applying NAS security.");
      return;
...
void nas::send_esm_information_response(const uint8 proc_transaction_id)
{
  ...
  logging_ul_message(tmsg.logfile.c_str(), "ESM Information Response");
  rrc->write_sdu(std::move(pdu));
                                                            ...
                                                            const uint8_t& eps_bearer_id)
{
  ...
  logging_ul_message(tmsg.logfile.c_str(), "Activate Dedicated EPS Bearer Context Accept");            
  rrc->write_sdu(std::move(pdu));
...
void nas::send_deactivate_eps_bearer_context_accept(const uint8_t& proc_transaction_id, const uint8_t& eps_bearer_id)
{
  ...
  logger.info(pdu->msg,
              ...
              accept.eps_bearer_id,
              accept.proc_transaction_id);
  logging_ul_message(tmsg.logfile.c_str(), "Seactivate EPS Bearer Context Accept");
  rrc->write_sdu(std::move(pdu));
...
void nas::send_modify_eps_bearer_context_accept(const uint8_t& proc_transaction_id, const uint8_t& eps_bearer_id)
{
  ...
  logger.info(pdu->msg,
              ...
              accept.eps_bearer_id,
              accept.proc_transaction_id);
  logging_ul_message(tmsg.logfile.c_str(), "Modify EPS Bearer Context Accept");
  rrc->write_sdu(std::move(pdu));
...
void nas::send_activate_test_mode_complete()
{
  ...
  logging_ul_message(tmsg.logfile.c_str(), "Activate Test Mode Complete");
  rrc->write_sdu(std::move(pdu));
...
void nas::send_close_ue_test_loop_complete()
{
  ...
  logging_ul_message(tmsg.logfile.c_str(), "Close UE Test Loop Complete");
  rrc->write_sdu(std::move(pdu));
...
bool nas::write_ctxt_file(nas_sec_ctxt ctxt_, nas_sec_base_ctxt ctxt_base_)
{
  ...
  srsran::console("Write ctxt File\n");
  if (file.is_open()) {
...
//logging function
void nas::logging_message(std::string file, std::string message, std::string notes)
{
  ...
  char timeString[9]; // 8 characters for HH:MM:SS + null terminator
  std::strftime(timeString, sizeof(timeString), "%H:%M:%S", timeInfo);
    // Print the current time
  log_stream << timeString << " : " << message << " " << notes << std::endl;
}

//test message parsing and control
bool nas::control_test_message()
{
  parse_msg_conf();
  ...
  else if (tmsg.conf_map["msg_type"].compare("attach_request") == 0)
  {
    srsran::console("Testing Initial Msg.: Attach Request\n");
    return false;
    // send_attach_request();
  }
  else if (tmsg.conf_map["msg_type"].compare("service_request") == 0)
  {
    srsran::console("Testing Initial Msg.: Service Request\n");
    state.set_registered(emm_state_t::registered_substate_t::normal_service);
    start_service_request(srsran::establishment_cause_t::mo_data);
  }
  else if (tmsg.conf_map["msg_type"].compare("tau_request") == 0)
  {
    srsran::console("Testing Initial Msg.: TAU Request\n");
    start_tau_request(srsran::establishment_cause_t::mo_sig);
  }
  else if (tmsg.conf_map["msg_type"].compare("authentication_failure") == 0)
  {
    send_authentication_failure(LIBLTE_MME_EMM_CAUSE_MAC_FAILURE, tmsg.auth_res);
  }
  else if (tmsg.conf_map["msg_type"].compare("authentication_response") == 0)
  {
    send_authentication_response(tmsg.auth_res, tmsg.auth_res_len);
  }
  else if (tmsg.conf_map["msg_type"].compare("identity_response") == 0)
  {
    send_identity_response(tmsg.id_type);
  }
  else if (tmsg.conf_map["msg_type"].compare("detach_request") == 0)
  {
    srsran::console("Testing Initial Msg.: Detach Request\n");    
    send_detach_request(true);
  }
  else if (tmsg.conf_map["msg_type"].compare("attach_complete") == 0)
  {
    send_attach_complete(tmsg.transaction_id, tmsg.bearer_id);
  }
  else if (tmsg.conf_map["msg_type"].compare("security_mode_reject") == 0)
  {
    send_security_mode_reject(tmsg.smc_reject_cause);
  }
  else if (tmsg.conf_map["msg_type"].compare("security_mode_complete") == 0)
  {
    send_security_mode_complete(tmsg.imeisv_present);
  }
  else
  {
    srsran::console("[error] Invalid Msg Type \'%s\'\n", tmsg.conf_map["msg_type"].c_str());
  }
