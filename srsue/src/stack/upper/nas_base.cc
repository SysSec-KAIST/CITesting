...
bool nas_base::integrity_check(byte_buffer_t* pdu)
{
  ...
  if (pdu->N_bytes > seq_offset) {
    ...
    for (int i = 0; i < 4; i++) {
      if (exp_mac[i] != mac[i]) {
        logger.warning("Integrity check failure. Local: count=%d, [%02x %02x %02x %02x], "
                       ...
                       mac[2],
                       mac[3]);
        printf("Integrity check failure. Local: count=%d, [%02x %02x %02x %02x], "
                       "Received: count=%d, [%02x %02x %02x %02x]\n",
                       count_est,
                       exp_mac[0],
                       exp_mac[1],
    ...
    logger.info("Integrity check ok. Local: count=%d, Received: count=%d  [%02x %02x %02x %02x]",
                ...
                mac[2],
                mac[3]);
    printf("Integrity check ok. Local: count=%d, Received: count=%d  [%02x %02x %02x %02x]\n",
                count_est,
                pdu->msg[seq_offset],
                mac[0],
                mac[1],
    ...
    if (count_est != ctxt_base.rx_count) {
      printf("[integrity_check] count_est update\n");
