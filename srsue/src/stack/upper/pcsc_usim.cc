...
int pcsc_usim::init(usim_args_t* args)
{
  ...
  //add imsi_a and imei_a from conf file
  imsi_a_str = args->imsi_a;
  imei_a_str = args->imei_a;
  imsi_tmp_str = args->imsi_tmp;
  tmsi_tmp = args->tmsi_tmp;
  ...
  const char* imsi_a_c = args->imsi_a.c_str();
  const char* imei_a_c = args->imei_a.c_str();
  const char* imsi_tmp_c = args->imsi_tmp.c_str();
  // const char* tmsi_tmp_c = args->tmsi_tmp.c_str();

  if (15 == args->imsi_a.length()) {
    imsi_a = 0;
    for (int i = 0; i < 15; i++) {
      imsi_a *= 10;
      imsi_a += imsi_a_c[i] - '0';
    ...
    logger.error("Invalid length for IMSI: %zu should be %d", args->imsi_a.length(), 15);
    srsran::console("Invalid length for IMSI: %zu should be %d\n", args->imsi_a.length(), 15);
  }

  if (15 == args->imei_a.length()) {
    imei_a = 0;
    for (int i = 0; i < 15; i++) {
      imei_a *= 10;
      imei_a += imei_a_c[i] - '0';
    ...
    logger.error("Invalid length for IMEI: %zu should be %d", args->imei_a.length(), 15);
    srsran::console("Invalid length for IMEI: %zu should be %d\n", args->imei_a.length(), 15);
  }

  if (15 == args->imsi_tmp.length()) {
    imsi_tmp = 0;
    for (int i = 0; i < 15; i++) {
      imsi_tmp *= 10;
      imsi_tmp += imsi_tmp_c[i] - '0';
    }
  } else {
    logger.error("Invalid length for Temperary IMSI: %zu should be %d", args->imsi_tmp.length(), 15);
    srsran::console("Invalid length for Temperary IMSI: %zu should be %d\n", args->imsi_tmp.length(), 15);
  }

  printf("'pcsc_usim' init tmsi_tmp: %d", tmsi_tmp);
                                                          ...
                                                          uint8_t* k_asme)
{
  ...
  uint8_t sqn[SQN_LEN];
  //srsran::console("Auth SQN::::::::\n");
  for (int i = 0; i < 6; i++) {
    sqn[i] = autn_enb[i] ^ ak[i];
    //Authentication sqn print
    //srsran::console("%02x ", sqn[i]);
  }
  //srsran::console("\n\n");
