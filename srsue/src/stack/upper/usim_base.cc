...
bool usim_base::get_imsi_tmp_vec(uint8_t* imsi_, uint32_t n)
{
  if (!initiated) {
...
//for imsi_a and imei_a
std::string usim_base::get_imsi_a_str()
{
  return imsi_a_str;
}
std::string usim_base::get_imei_a_str()
{
  return imei_a_str;
}

std::string usim_base::get_imsi_tmp_str()
{
  return imsi_tmp_str;
}

uint32_t usim_base::get_tmsi_tmp()
{
  return tmsi_tmp;
}

bool usim_base::get_imsi_a_vec(uint8_t* imsi_, uint32_t n)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return false;
...
bool usim_base::get_imei_a_vec(uint8_t* imei_, uint32_t n)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return false;
